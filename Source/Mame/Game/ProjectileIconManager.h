...
public:
    void Register(ProjectileIcon* projectile);  // 登録
    void Remove(ProjectileIcon* projectile);    // 削除
    void Clear();                           // 全削除

public: // 取得・設定　関連
    const int GetProjectileCount() const { return static_cast<int>(projectileIcons.size()); } // 弾丸数取得

    ProjectileIcon* GetProjectile(int index) { return projectileIcons.at(index); } // 弾丸取得
