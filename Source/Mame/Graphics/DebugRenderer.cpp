...
void DebugRenderer::Render(ID3D11DeviceContext* context, const DirectX::XMFLOAT4X4& view, const DirectX::XMFLOAT4X4& projection)
{
	...
	{
		// ワールドビュープロジェクション行列作成
		const DirectX::XMMATRIX S = DirectX::XMMatrixScaling(box.scale.x, box.scale.y, box.scale.z);
		//// 回転行列を作成
		//DirectX::XMMATRIX R = DirectX::XMMatrixRotationRollPitchYaw(angle.x, angle.y, angle.z);
		DirectX::XMMATRIX X = DirectX::XMMatrixRotationX(box.rotation.x);
		DirectX::XMMATRIX Y = DirectX::XMMatrixRotationY(box.rotation.y);
		DirectX::XMMATRIX Z = DirectX::XMMatrixRotationZ(box.rotation.z);
		// 先にX軸、Z軸の回転を確定させてからY軸を回転させる
		//（右から順番に掛けられる）
		DirectX::XMMATRIX R = Y * X * Z;
		const DirectX::XMMATRIX T = DirectX::XMMatrixTranslation(box.center.x, box.center.y, box.center.z);
...
// 箱メッシュ作成
void DebugRenderer::CreateBoxMesh(ID3D11Device* device, const float& size)
{
	boxVertexCount = 24;
